...
using TakTikan.Tailor.Migrations.Seed.Host;
...
namespace TakTikan.Tailor.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : TailorAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<TailorDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{TailorConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{TailorConsts.ConnectionStringName}", constring);
        }
        ...
        private async Task SetAdminPassword(string adminPassword)
        {
            ...
            var loginResult = await _logInManager.LoginAsync(User.AdminUserName, "kronos");
